...
func Trace(pid int) (*TracedProgram, error) {
	...
	for {
		...
		if err != nil {
			fmt.Println(err, "read failed", "pid", pid)
		...
		for _, thread := range threads {
			...
			if err != nil {
				...
				if retryCount[tid] < threadRetryLimit {
					fmt.Println("retry attaching thread", "tid", tid, "retryCount", retryCount[tid], "limit", threadRetryLimit)
					continue
				}

				if !strings.Contains(err.Error(), "no such process") {
					fmt.Println(err, "attach failed", "tid", tid)
					return nil, errors.WithStack(err)
				}
				continue
			}
